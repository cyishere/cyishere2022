...
By using the keyboard to navigate it, we need some JavaScript. First, add `aria-hidden` to hide all the slide items from the accessibility devices. In this React app, I used `useRef` to access those `div` slide items.
...
Next, by removing those added attributes, I made the **current** slide "visible" to accessibility devices.
