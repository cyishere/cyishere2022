...
export const meta = {
  ...
  pageImage:
    "https://res.cloudinary.com/cyishere/image/upload/v1649225933/cyishere/blog/make-graphql-codegen-work-with-hasura-in-nextjs/cover_ox8bgs.png",
};
...
As I used the <ExternalLink href="https://www.graphql-code-generator.com/">GraphQL Code Generator</ExternalLink> before, I found out that making this Codegen work with <ExternalLink href="https://hasura.io">Hasura</ExternalLink> in <ExternalLink href="https://nextjs.org/">Next.js</ExternalLink> needs some special configurations.

## Set Codegen Configurations for Hasura

<ExternalLink href="https://hasura.io/learn/graphql/typescript-react-apollo/codegen/">
  Hasura's official tutorial
</ExternalLink> provides a settings code block which I modified some "special configurations"
in it (with the ⛑️ indicator):

```javascript
// ⛑️ Change 1: import "dotenv"
require("dotenv").config();

// ⛑️ Change 2: Get Hasura's GraphQL API from enviroment variables
const API = process.env.HASURA_GRAPHQL_API;

module.exports = {
  schema: [
    {
      // ⛑️ Change 3: Use the `API` varialbe that just defined
      [API]: {
        headers: {
          // ⛑️ Change 4: Add content type
          "content-type": "application/json",
          // ⛑️ Change 5: Add admin secret fro enviroment variables
...
- Change 1: for this `codegen.js` file can get environment variables, we need to install <ExternalLink href="https://www.npmjs.com/package/dotenv">dotenv</ExternalLink> and import it into this file.
...
In the picture above, at default Hasura GraphQL API needs `content-type` and `x-hasura-admin-secret`. **This is why I make the "4" and "5" changes.** After setting the permissions in the database tables, we can interact with the API with `x-hasura-role` and `x-hasura-user-id`. More "permission" content could find in <ExternalLink href="https://hasura.io/learn/graphql/hasura/authorization/1-todos-table-permissions/">their official tutorials</ExternalLink>.
...
This is a note from me about developing an application with Next.js and Hasura. The app is called DDD App, and it is a tool for remembering German nouns. You can learn more about it [here](/blog/ddd-app-a-neat-tool-for-remembering-german-nouns).
