...
As the `next-mdx-remote` [official document says](https://www.npmjs.com/package/rehype-prism-plus) when we serialize the MDX content, we can use `remarkPlugins` and `rehypePlugins`. `rehype-prism-plus` is one of the "rehypePlugins".
