...
This article was first written in 2022, when Astro was very new. Now in 2025, it has evolved significantly and has a new built-in way to achieve the active state. The code is shown below, but I'll keep the older methods for reference.
...
<a class={pathname.slice(1) === slug ? 'active' : ''} href={`/${slub}`}>
  About
</a>
```
